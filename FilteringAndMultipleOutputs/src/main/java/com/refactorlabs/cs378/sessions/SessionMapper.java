...
public class SessionMapper extends Mapper<AvroKey<CharSequence>, AvroValue<Session>, AvroKey<CharSequence>, AvroValue<Session>> {
    ...
    public void map(AvroKey<CharSequence> key, AvroValue<Session> value, Context context)
        ...
        if(events.size() > 100){
            context.getCounter(Utils.LARGE_SESSION_COUNT, "Discarded Large Session").increment(1L);
            userLevel = 6;
        }
        //if is not large session, have to iterate through events to determine final level
            ...
            for (Event event : events){
                    ...
                    //if event type is click and current level value is less than click, reset level as 2
                    if((event.getEventType() == EventType.CLICK) && userLevel > 2)
                        userLevel = 2; 
                    //if event type is show or display, and current level value is less than 3, reset level as 2
        ...
        if (userLevel == 2){
            if (rand.nextInt(100)+1 < 11)
                multipleOutputs.write(SessionType.CLICKER.getText(), key, value);
            else
                context.getCounter(Utils.CLICK_SESSION_COUNT, "Discarded Clicker Session Counts").increment(1L);
        }
        //when level is 3
        ...
        if (userLevel == 3){
            if (rand.nextInt(100)+1 < 3)
                ...
                context.getCounter(Utils.SHOWER_SESSION_COUNT, "Discarded Shower Session Counts").increment(1L);
        }
